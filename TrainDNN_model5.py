...
dir_model_logger = dir_mofiles+"/"+modelname+".h5"
...
# load model
t.tic()
mymodel = load_model(dir_mofiles+"/DNN_"+modelname+".h5",custom_objects={'GlorotUniform': glorot_uniform(), "cust_mean_squared_error": cust_mean_squared_error})
mymodel.summary()
...
# train the model
t.tic()
history = mymodel.fit([an_l_rand_train, an_r_rand_train], labels_rand_train, validation_data=((an_l_rand_test,an_r_rand_test),labels_rand_test), epochs = nrepochs, batch_size = sizebatches, verbose = 1, use_multiprocessing = True, callbacks = [csv_loss_logger, model_logger])
