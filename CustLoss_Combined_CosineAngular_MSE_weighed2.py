...
# two fake points
def cos_dist_angular_and_mse_weighed2(y_true,y_pred):
    ...
    cosine_distance_angular = tf.acos(K.clip(cos_sim,-1+K.epsilon(),1-K.epsilon()))/3.14159265359
    ...
    my_loss = MSE+cosine_distance_angular # by assigning both the weight one, MSE is weighed twice as much because MSE ranges between 0 and 2, while angular distance ranges between 0 and 1
